  ...
  for (int j = 0; j < _gridSize[1]; j++)
    for (int i = 0; i < _gridSize[0]; i++) {
      ...
      if (i < _gridSize[0])
        ...
        gradient[id] = (phi[id] - phi[ijToid(i - 1, j)]) / h[0];

      if (j < _gridSize[1])
        gradient[id][1] = (phi[ijToid(i, j + 1)] - phi[id]) / h[1];
      else
        gradient[id] = (phi[id] - phi[ijToid(i, j - 1)]) / h[1];
    }
}
...
#pragma omp parallel for
    for (int id = 0; id < cellCount(); id++) {
      ...
      if (i < _gridSize[0] - 1)
        dx[1] = direction[id][0] = (phi[ijToid(i + 1, j)] - phi[id]) / h[0];
      if (i > 0)
        dx[0] = direction[id][0] = (phi[id] - phi[ijToid(i - 1, j)]) / h[0];

      if (j < _gridSize[1] - 1)
        dy[1] = direction[id][1] = (phi[ijToid(i, j + 1)] - phi[id]) / h[1];
      if (j > 0)
        dy[0] = direction[id][1] = (phi[id] - phi[ijToid(i, j - 1)]) / h[1];
